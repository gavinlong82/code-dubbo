...
public class MultiDataSourceAspect {
	...
	/**
	 * 功能：设置数据源 DataSource注解方法有效，无注解则使用默认数据源（主数据源）。 value为数据源的key，与name相同
	 * manual为true时，需要方法的第一个参数为数据源的key。默认为false。
	 * 
	 * @param point
	 *            方法切点
	 */
	@Before("execution(* com.ytd.*.dao.*.*(..))")
	public void before(JoinPoint point) {
		String methodName = point.getSignature().getName(); // 方法名
		Class<?>[] parameterTypes = ((MethodSignature) point.getSignature()).getMethod().getParameterTypes();
		try {
			// 默认使用类注解
			if (point.getTarget().getClass().isAnnotationPresent(DataSource.class)) {
				logger.info("MultiDataSourceAspect Class Name:" + point.getTarget().getClass().getName());
				DataSource source = point.getTarget().getClass().getAnnotation(DataSource.class);
				DynamicDataSourceHolder.putDataSource(source.value());
			}

			Method method = point.getTarget().getClass().getMethod(methodName, parameterTypes);
			logger.info("MultiDataSourceAspect Method Name:" + method.getName());
			if (method.isAnnotationPresent(DataSource.class)) { // 指定某个数据源
				...
				if (dataSource.manual())// 程序人工指定则从方法参数的第一个选取。默认不特殊制定
					value = point.getArgs().length > 0 ? point.getArgs()[0] + "" : MdsConstants.MASTER_DATASOURCE; // 方法参数的第一个值为数据源name
