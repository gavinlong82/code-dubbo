...
import com.ytd.mds.config.mybatis.MyBatisConfig;
...
public class DynamicDataSource extends AbstractRoutingDataSource {
	...
	@Autowired
	@Qualifier(MdsConstants.MASTER_DATASOURCE)
	private DruidDataSource masterDataSource;

	@Autowired
	@Qualifier(MdsConstants.SLAVE1_DATASOURCE)
	private DruidDataSource slave1DataSource;

	@Autowired
	@Qualifier(MdsConstants.SLAVE2_DATASOURCE)
	...
	protected Object determineCurrentLookupKey() {
		...
		if (MdsConstants.SLAVE_DATASOURCE_CODE.equals(key)) // 从机代号，则在从机集群中选出一个
	...
	public void afterPropertiesSet() {
		...
		try {
			...
			for (Entry<Object, DataSource> entry : resolvedDataSources.entrySet()) {
				if (MdsConstants.MASTER_DATASOURCE.equals(entry.getKey())) {
